...
| `/writing/[slug]` | Single note | `Note.dc.html` (full spec in `WRITING_PAGES_DESIGN.md`) |
...
- **The logo mark.** The favicon is the exact header logo. Use this one SVG everywhere (header, badge centre, favicon):

  ```svg
  <svg viewBox="0 0 42 42" xmlns="http://www.w3.org/2000/svg">
    <circle cx="21" cy="21" r="21" fill="#1F2AD6"/>
    <path d="M12 28 L19 13 L26 28" fill="none" stroke="#F7F7F5" stroke-width="2.6" stroke-linecap="round" stroke-linejoin="round"/>
    <path d="M31 16.5c-1-1.6-2.6-2.3-4.3-2.3" fill="none" stroke="#F7F7F5" stroke-width="2.6" stroke-linecap="round"/>
    <circle cx="31" cy="26.5" r="2.4" fill="#AEB5FF"/>
  </svg>
  ```

  - The same file is in `brand-assets/icon.svg`.
  - **Inverted version** (header over the blue hero, mobile header): circle `#F7F7F5`, lines `#1F2AD6`, dot `#AEB5FF`.
  - Do not redraw or change the mark.
...
The full, current spec for the notes list and single note pages is in **`WRITING_PAGES_DESIGN.md`**. It replaces the short summary below.
...
## 7a. How the site looks in Google (site name and favicon)

**Goal:** in Google results the top line should read **Ajaypal Singh** with the cobalt "A" favicon, not `ajaypalsingh.in` with a grey globe. When Google has no site-name signal or favicon, it falls back to the domain and a globe icon. The second line (`https://ajaypalsingh.in › ...`) is the URL breadcrumb. Google always shows that line, for Instagram too.

### Favicon files (in `brand-assets/`)

Copy these into `/public` at the site root:

| File | Size | Used for |
|---|---|---|
| `favicon.ico` | 16/32/48 | Browsers, Google |
| `icon.svg` | vector | Modern browsers |
| `icon-48.png`, `icon-96.png` | 48, 96 | Google Search (square, at least 48px) |
| `icon-192.png`, `icon-512.png` | 192, 512 | Android, web manifest |
| `apple-touch-icon.png` | 180 | iOS home screen (full square, because iOS rounds the corners itself) |

**Rules**
- The icons are the same round mark as the site logo, with a transparent background outside the circle.
- The icons must be square.
- The URLs must not change.
- The files and the homepage must not be blocked in `robots.txt`.
- The `<link>` tags must be on the **homepage** (`/`).
...
export const metadata: Metadata = {
  ...
  title: { default: 'Ajaypal Singh', template: '%s | Ajaypal Singh' },
  description: 'Ajaypal Singh is a founder, builder and entrepreneur. Building Ojaven.',
  applicationName: 'Ajaypal Singh',
  alternates: { canonical: '/' },
  openGraph: { siteName: 'Ajaypal Singh', type: 'website', url: 'https://ajaypalsingh.in' },
  icons: {
    icon: [
...
Page titles:
- **Homepage:** `Ajaypal Singh`. It can be `Ajaypal Singh: Founder, Builder, Entrepreneur`, but it should start with the name.
- **Other pages:** `About | Ajaypal Singh`, `Ojaven | Ajaypal Singh` and so on.

### Structured data: homepage only, one `WebSite` node
...
- The homepage `<h1>`, the header text and `og:site_name` all say "Ajaypal Singh".
- If both `www.ajaypalsingh.in` and `ajaypalsingh.in` work, redirect one to the other.
- If a duplicate homepage still exists, it needs the same structured data.

### Plain HTML version (any other stack)

```html
<title>Ajaypal Singh</title>
<meta property="og:site_name" content="Ajaypal Singh">
<link rel="icon" href="/favicon.ico" sizes="48x48">
<link rel="icon" href="/icon.svg" type="image/svg+xml">
<link rel="icon" href="/icon-96.png" sizes="96x96" type="image/png">
<link rel="apple-touch-icon" href="/apple-touch-icon.png">
<script type="application/ld+json">
{"@context":"https://schema.org","@type":"WebSite","name":"Ajaypal Singh","alternateName":["Ajaypal","ajaypalsingh.in"],"url":"https://ajaypalsingh.in/"}
</script>
```

### After deploying

1. Test the homepage in Google's Rich Results Test. `WebSite` should be detected.
2. In Search Console, use URL Inspection on `https://ajaypalsingh.in/` and click **Request indexing**.
3. Wait. Updates can take from a few days to several weeks. Google may show the new name on the homepage result first and on inner pages like `/ojaven` later.
4. Google decides automatically, so neither the name nor the favicon is guaranteed. These steps give it the strongest signals.
